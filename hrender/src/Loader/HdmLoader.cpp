...
template <typename T>
void LoadBlockCellVector(VoxelBlock<T>& block, Scalar3D<T>* U, Scalar3D<T>* V, Scalar3D<T>* W, Vec3i sz, size_t level, size_t maxLevel, size_t rootDim[3], const Vec3d& org, const Vec3d& globalOrigin, const Vec3d& pitch, const Vec3d& globalRegion) {
		...
		for(size_t y = 0; y < sz.y; y++){
			for(size_t x = 0; x < sz.x; x++){
				...
				for (size_t w = 0; w < bz; w++) {
					for (size_t v = 0; v < by; v++) {
						for (size_t u = 0; u < bx; u++) {
							block.data[3 * ((z * bz + w) * dy * dx + (y * by + v) * dx + (x * bx + u)) + 0] = valU;
							block.data[3 * ((z * bz + w) * dy * dx + (y * by + v) * dx + (x * bx + u)) + 0] = valV;
							block.data[3 * ((z * bz + w) * dy * dx + (y * by + v) * dx + (x * bx + u)) + 0] = valW;
						}
					}
				}
...
template <typename T>
void ConvertLeafBlockVector(BufferSparseVolumeData& sparseVolume, const int dcid[3], size_t maxLevel, size_t rootDim[3], const Vec3d& globalOrigin, const Vec3d& globalRegion){
    BlockManager& blockManager = BlockManager::getInstance();
    const MPI::Intracomm& comm = blockManager.getCommunicator();
    Vec3i sz = blockManager.getSize();
    ...
    for(int id = 0; id < blockManager.getNumBlock(); ++id){
        ...
        Scalar3D<T> *meshU = dynamic_cast< Scalar3D<T>* >(block->getDataClass(dcid[0]));
        Scalar3D<T> *meshV = dynamic_cast< Scalar3D<T>* >(block->getDataClass(dcid[1]));
        Scalar3D<T> *meshW = dynamic_cast< Scalar3D<T>* >(block->getDataClass(dcid[2]));

		VoxelBlock<T> vb;
		LoadBlockCellVector(vb, meshU, meshV, meshW, sz, level, maxLevel, rootDim, org, globalOrigin, pitch, globalRegion);
		vb.id = id;
	...
	std::list<unsigned int>::const_iterator it = stepList->begin();
	{
		...
		if (type == TYPE_FLOAT32) {
			...
			if (components == 1) {
				...
				ConvertLeafBlockVector<float>(m_sparseVolume, id_v32, maxLevel, rootDim, globalOrigin, globalRegion);
			}

		} else if (type == TYPE_FLOAT64) {
			if (components == 1) {
				loader.LoadLeafBlock(&id_s64, fieldName, vc, *it);
				ConvertLeafBlockScalar<double>(m_sparseVolume, id_s64, maxLevel, rootDim, globalOrigin, globalRegion);
			} else {
				loader.LoadLeafBlock(id_v64, fieldName, vc, *it);
				ConvertLeafBlockVector<double>(m_sparseVolume, id_v64, maxLevel, rootDim, globalOrigin, globalRegion);
			}
		}

	}

	// @note { Must call `Build` here to build spacial data structure for sparse volume(to use `Sample` meethod in later phase. }
