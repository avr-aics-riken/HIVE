...
{
	std::stringstream ss;
	ss << "{\"jsonrpc\":\"2.0\", \"result\":\"" << result << "\", \"id\":\""
	   << id << "\", \"to\":\"client\"}";
	return ss.str();
}

std::string makeMasterRegistMethod(const std::string &method,
								   const std::string &targetid)
{
	std::stringstream ss;
	ss << "{\"jsonrpc\":\"2.0\", \"method\":\"" << method
	   << "\", \"param\":{\"mode\":\"renderer\", \"targetid\":\"" << targetid
	   << "\"}, \"to\":\"master\"}";
	return ss.str();
}
 ...
 * server mode関数
 */
/*void serverMode(const std::string& url, const std::vector<std::string>&
sceneargs)
{
...
// printf("[MPI] rank = %d\n", rank);
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		if (i == 1 && na > 4 &&
			(strncasecmp(&argv[i][na - 4], ".scn", 4) == 0 ||
			 strncasecmp(&argv[i][na - 4], ".lua", 4) == 0))
		{
			scenefile = argv[i];
		}
