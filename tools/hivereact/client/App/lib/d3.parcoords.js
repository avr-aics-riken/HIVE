d3.parcoords = function(config) {
    ...
    // custom
    if(config.usr != null){
        for(var key in config.usr){
            if(pc[key] == null){
                pc[key] = config.usr[key];
            }
        }
    }
            ...
            "number": function(k) {
                ...
                if(config.usr.customScale &&
                   ...
                   config.usr.max !== null && config.usr.max !== undefined){
                    ...
                    __.extent = [config.usr.min, config.usr.max];
                }else{
                    __.extent = [extent[0], extent[1]];
                }
    ...
    function glData(v, target){
        ...
        for(i = 0, j = v.length; i < j; ++i){
            ...
            c = [];
            for(l = 0; l < k; ++l){
                c[l] = {colCount: l, colPosition: position(l)};
            }
            c.sort(function(a, b){
                if(a.colPosition < b.colPosition){return -1;}
                if(a.colPosition > b.colPosition){return  1;}
                return 0;
            });
            left = c[0].colPosition;
            right = c[c.length - 1].colPosition;
            d = [];
            ...
            for(l = 0; l < k; ++l){
                ...
                x = position(d[l]);
                y = yscale[d[l]](s);
                if(s >= __.extent[0] && s <= __.extent[1]){
                    if(l <= 1){
                        e.push(x, y);
                    }else{
                        m = (l - 1) * 4;
