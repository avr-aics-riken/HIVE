...
namespace {
    ...
    public:
        ...
        void searchNearestSolid(const VX::Math::vec3 &point, float &d0, int node_id){
            
            if(tree[node_id].leaf != -1){
                d0 = std::min(getDistanceSldPt(point, solids[tree[node_id].leaf]), d0);
                return;
            }
            
            float d1 = getDistance(tree[tree[node_id].children[0]], point),
            d2 = getDistance(tree[tree[node_id].children[1]], point);
            if(d1 < d2){
                if(d1 < d0)
                    searchNearestSolid(point, d0, tree[node_id].children[0]);
                if(d2 < d0)
                    searchNearestSolid(point, d0, tree[node_id].children[1]);
            }else {
                if(d2 < d0)
                    searchNearestSolid(point, d0, tree[node_id].children[1]);
                if(d1 < d0)
 ...
 * @retval 0 変換失敗
 */
int SolidDfToVolume::Create(BufferSolidData *solid, float *bmin, float *bmax){
    if (!solid) {
        return 0;
    }
 ...
 */
BufferVolumeData* SolidDfToVolume::VolumeData()
{
    return m_volume;
}

/// コンストラクタ
SolidDfToVolume::SolidDfToVolume() : m_volume(0), m_solid(0) {
}

/// デストラクタ
SolidDfToVolume::~SolidDfToVolume() {
}
