...
int RGBE_WritePixels_RLE(FILE *fp, const float *data, int scanline_width,
						 int num_scanlines);
...
inline int RGBE_ReadHeader(FILE *fp, int *width, int *height, rgbe_header_info *info)
{
	char buf[128];
	float tempf;
	int i;
...
inline int RGBE_WritePixels(FILE *fp, const float *data, int numpixels)
{
...
inline int RGBE_ReadPixels(FILE *fp, float *data, int numpixels)
{
	unsigned char rgbe[4];
...
static inline int RGBE_WriteBytes_RLE(FILE *fp, unsigned char *data, int numbytes)
{
...
inline int RGBE_WritePixels_RLE(FILE *fp, const float *data, int scanline_width,
						 int num_scanlines)
{
	unsigned char rgbe[4];
...
inline int RGBE_ReadPixels_RLE(FILE *fp, float *data, int scanline_width,
						int num_scanlines)
{
...
inline bool SimpleHDRLoader(const char* hdrfilename, int& w, int& h, float** rgba_float)
{
	FILE* fp = fopen(hdrfilename, "rb");
	if (!fp)
		return false;
	int r = SimpleHDR::RGBE_ReadHeader(fp, &w, &h, NULL);
	if (r) {
		fclose(fp);
		return false;
	}
...
inline bool SimpleHDRSaver(const char* hdrfilename, int w, int h, const float* rgba_float)
{
	FILE* fp = fopen(hdrfilename, "wb");
	if (!fp)
		return false;
	SimpleHDR::RGBE_WriteHeader(fp, w, h, NULL);
	SimpleHDR::RGBE_WritePixels_RLE(fp, rgba_float, w, h);
	fclose(fp);
	return true;
}
